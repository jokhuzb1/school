...
import { Card, Row, Col, Avatar, Typography, Statistic, Table, Tag, Calendar, Badge, Spin, Empty } from 'antd';
...
const StudentDetail: React.FC = () => {
    ...
    const columns = [
        ...
        { title: 'Arrived', dataIndex: 'firstScanTime', key: 'arrived', render: (t: string) => t ? dayjs(t).format('HH:mm') : '-' },
        { title: 'Left', dataIndex: 'lastScanTime', key: 'left', render: (t: string) => t ? dayjs(t).format('HH:mm') : '-' },
        ...
        <div>
            <Card style={{ marginBottom: 16 }}>
                <Row gutter={24} align="middle">
                    <Col>
                        <Avatar size={80} src={student.photoUrl} icon={<UserOutlined />} />
                    </Col>
                    <Col>
                        ...
                        <br />
                        <Text>Parent: {student.parentName || '-'} ({student.parentPhone || '-'})</Text>
                    </Col>
                </Row>
            </Card>

            <Row gutter={16} style={{ marginBottom: 16 }}>
                <Col xs={12} sm={6}><Card><Statistic title="Total Days" value={stats.total} /></Card></Col>
                <Col xs={12} sm={6}><Card><Statistic title="Present" value={stats.present} styles={{ value: { color: '#52c41a' } }} /></Card></Col>
                <Col xs={12} sm={6}><Card><Statistic title="Late" value={stats.late} styles={{ value: { color: '#faad14' } }} /></Card></Col>
                <Col xs={12} sm={6}><Card><Statistic title="Absent" value={stats.absent} styles={{ value: { color: '#ff4d4f' } }} /></Card></Col>
            </Row>
            ...
            <Card title="Attendance History">
                <Table dataSource={attendance} columns={columns} rowKey="id" size="small" />
            </Card>
        </div>
    );
};
